defmodule CTransferWeb.InvoiceLive.Index do
  @moduledoc false
  ...
  def mount(_params, _session, socket) do
    {:ok, stream(socket, :invoices, Transactions.list_invoices())}
  end
  ...
  @impl true
  def handle_info({CTransferWeb.InvoiceLive.FormComponent, {:saved, invoice}}, socket) do
    {:noreply, stream_insert(socket, :invoices, invoice)}
  end

  @impl true
  def handle_event("delete", %{"id" => id}, socket) do
    invoice = Transactions.get_invoice!(id)
    {:ok, _} = Transactions.delete_invoice(invoice)
