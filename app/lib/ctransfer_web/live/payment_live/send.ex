defmodule CTransferWeb.PaymentLive.Send do
  @moduledoc false
  ...
  def render(assigns) do
    ...
    <.list>
      ...
      <:item title="Transaction">
        <.link href={"https://solana.fm/tx/#{@payment.tx_hash}" <> if Application.get_env(:ctransfer, :network) && Application.get_env(:ctransfer, :network) == :devnet, do: "?cluster=devnet-solana", else: ""}>
          <%= @payment.tx_hash %>
        </.link>
      </:item>
    </.list>
